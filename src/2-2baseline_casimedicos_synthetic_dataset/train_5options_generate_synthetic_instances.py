...
os.environ["WANDB_PROJECT"] = "tfg-baseline-casimedicos-synthetic"  # set the wandb project where this run will be logged
...
def main(training_arguments_path: str):
    ...
    # Preprecess function
    def generate_instances(examples):
        # For each example, generate 5 instances, one for each position for the correct answer (shuffle the options)
        # Get the only example in the list
        example = {"question": examples["question"][0], "label": examples["label"][0]} | {f"op{option}": examples[f"op{option}"][0] for option in label_list}

        # Get the correct answer
        correct_answer = id_to_label[example["label"]]

        # Drop the correct answer from the options
        incorrect_answers = label_list.copy()
        incorrect_answers.remove(correct_answer)

        # Generate 5 combinations of the incorrect answers
        synthetic_instances: dict[str, list] = {
            "question": [],
            "option_text": [],
            "label": []
        }
        for i in range(5):
            ...
            option_combination = incorrect_answers.copy()
            np.random.shuffle(option_combination)

            # Insert the correct answer in the position i
            option_combination.insert(i, correct_answer)

            # Generate the option text
            incorrect_combination = "</s>".join([example[f"op{option}"] for option in option_combination])

            # Create the synthetic instance
            synthetic_instances["question"].append(example["question"])
            synthetic_instances["option_text"].append(incorrect_combination)
            synthetic_instances["label"].append(i)
    ...
    dataset = dataset.map(generate_instances, batched=True, batch_size=1, remove_columns=dataset["train"].column_names)

    # Shuffle the dataset
    dataset = dataset.shuffle(seed=training_args.seed)
    dataset = dataset.map(tokenize_function, batched=True, load_from_cache_file=not model_data_args.overwrite_cache, )
