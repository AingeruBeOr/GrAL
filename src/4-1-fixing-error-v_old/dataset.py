...
class FixingErrorDataset(Dataset):
  '''
  Este dataset carga todas las instancias balanceadas de CasiMedicos y una instancia de MedMCQA
  '''  

  def __init__(self,
               jsonl_path_casimedicos,
               jsonl_path_medmcqa,
               use_context=False):
    self.use_context = use_context
    self.dataset = []
    ...
    # Load MedMCQA
    number_of_instances_to_load = 4
    loaded_instances = 0
    with open(jsonl_path_medmcqa, 'r') as file:
      for row in file:
        if loaded_instances != number_of_instances_to_load:
    ...
    # Load CasiMedicos
    with open(jsonl_path_casimedicos, 'r') as file:
      self.dataset += [json.loads(row) for row in file]
