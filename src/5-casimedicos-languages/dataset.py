...
class MedMCQA4_json(Dataset):
  '''
  Author: Aingeru
  '''
  def __init__(self, medmcqa_path):
    with open(medmcqa_path, 'r') as file:
...
class MedMCQAAndCasiMedicosDataset(Dataset):
  '''
  Author: Aingeru
  Description: Used for training with both MedMCQA and CasiMedicos datasets
  '''

  def __init__(self,
               medmcqa_dataset_path,
               casimedicos_dataset_path,
               use_context=False):
    self.use_context = use_context
    self.dataset = []

    # Load MedMCQA
    with open(medmcqa_dataset_path, 'r') as file:
      for row in file:
        ...
        new_instance = {
          ...
          'correct_option': instance['cop']
        }
        self.dataset.append(new_instance)
    
    # Load CasiMedicos
    with open(casimedicos_dataset_path, 'r') as file:
      self.dataset += [json.loads(row) for row in file]
